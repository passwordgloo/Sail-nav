// js/links.js
export const linkGroups = [
    {
        id: '我的站点',
        icon: 'ri-apps-fill',
        links: [
            {
                title: '万象文库',
                desc: '森罗万象，百科全书',
                logo: '/images/wiki.webp',
                url: 'https://wiki.iglooblog.top'
            },
            {
                title: '彗星文库',
                desc: '法律法规资料库',
                logo: '/images/law.webp',
                url: 'https://law.iglooblog.top'
            },
            {
                title: '听潮公考',
                desc: '免费考公考编资料站',
                logo: '/images/tide.webp',
                url: 'https://tide.iglooblog.top'
            },
            {
                title: '云帆导航',
                desc: '去我的收藏夹吃灰吧',
                logo: '/images/nav.webp',
                url: 'https://nav.iglooblog.top'
            }
        ]
    },
    {
        id: '媒体',
        icon: 'ri-vidicon-fill',
        links: [
            {
                title: '博客',
                logo: '/images/blog.webp',
                url: 'https://iglooblog.top:82'
            },
            {
                title: '网盘',
                logo: '/images/cloud.webp',
                url: 'http://101.132.99.30:5212'
            },
            {
                title: '媒体',
                desc: '图书音乐影视',
                logo: '/images/bee.webp',
                url: 'https://media.iglooblog.top'
            }
        ]
    },
    {
        id: '实用工具',
        icon: 'ri-tools-fill',
        links: [
            {
                title: 'similar sites',
                desc: '同类型站点搜索',
                logo: 'https://favicon.im/similarsites.com',
                url: 'https://similarsites.com'
            },
            {
                title: 'flourish',
                desc: '数据可视化网站',
                logo: 'https://favicon.im/flourish.studio',
                url: 'https://flourish.studio'
            },
            {
                title: 'toolfk',
                desc: '一站式工具包',
                logo: 'https://favicon.im/toolfk.com',
                url: 'https://toolfk.com'
            }
        ]
    },
    {
        id: '设计网站',
        icon: 'ri-paint-brush-fill',
        links: [
            {
                title: 'Pinterest',
                desc: '全球最大的素材管理与分享社区',
                logo: 'https://favicon.im/pinterest.com',
                url: 'https://www.pinterest.com/'
            },
            {
                title: 'Dribbble',
                desc: '插画与UI设计交流社区',
                logo: 'https://favicon.im/dribbble.com',
                url: 'https://dribbble.com/shots'
            }
        ]
    },
    {
        id: '学习资源',
        icon: 'ri-mac-fill',
        links: [
            {
                title: 'classcentral',
                desc: '全球最大的课程学习网站',
                logo: 'https://favicon.im/classcentral.com',
                url: 'https://www.classcentral.com'
            },
            {
                title: 'lingohut',
                desc: '免费学习 45 种外语',
                logo: 'https://favicon.im/lingohut.com',
                url: 'https://www.lingohut.com/zh'
            }
        ]
    },
    {
        id: '设计素材',
        icon: 'ri-landscape-fill',
        links: [
            {
                title: 'Iconfont',
                desc: '阿里旗下图标交流社区',
                logo: 'https://favicon.im/iconfont.cn',
                url: 'https://www.iconfont.cn/'
            },
            {
                title: 'Unsplash',
                desc: '摄影师交流社区，免费可商用版权图片',
                logo: 'https://favicon.im/unsplash.com',
                url: 'https://unsplash.com/'
            },
            {
                title: 'Pexels',
                desc: '免费可商用图片社区',
                logo: 'https://favicon.im/pexels.com',
                url: 'https://www.pexels.com/zh-cn/'
            }
        ]
    },
    {
        id: 'AI工具',
        icon: 'ri-robot-fill',
        links: [
            {
                title: 'tree.ai',
                desc: '代码 AI 生成网站',
                logo: 'https://favicon.im/tree.ai',
                url: 'https://teee.ai'
            },
            {
                title: 'pai.video',
                desc: '支持 AI 文生视频',
                logo: 'https://favicon.im/pai.video',
                url: 'https://pai.video/onboard'
            }
        ]
    }
];
